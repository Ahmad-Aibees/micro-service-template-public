...
export class SystemIntegrationDbService {
    ...
    async registerService(): Promise<void> {
        ...
        await this.systemIntegrationModel.findOneAndUpdate({ label: process.env.service_label },{
            ...
            publicAddress: process.env.service_address ?? null,
            token,
            status: 'up',
            exitCode: null,
    ...
    async findActive(role: systemServiceRoleType): Promise<SystemIntegrationModel> {
        return this.systemIntegrationModel.findOne({ role }, undefined, { sort: { lastCheckIn: -1 } }).exec();
    }
}
